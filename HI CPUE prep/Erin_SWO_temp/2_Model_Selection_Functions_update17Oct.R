...
#	Modified to add glmer from lme4 for Swordfish longline
...
# FUNCTIONS --------------------------------------------------------------------------------------------------------------
# FORWARD SELECTION lowest AIC only if LR test sig / stop criteria = delta %dev. explained
...
#					additive terms have to be given (e.g. s(Hour, bs='cc')). specify k as well in each term to prevent
...
#  SAMPLE VALUES  commented out so they won't be read in.
...
#	var_name = c('Quarter_fac', 'Bait_fac', 'HPF_fac', 'Begin_fac', 'SST', 'MLD', 'Illum', 'SOI', 'PDO', 'Lat', 'Lon')
...
#	data_set <- SWOShalb_pos			#str(SWOShalb)			#str(SWOShala)
...
if (1 == 2) {
	var_name = c( "s(Lat)", 'HPSet_std', 'Bait_fac', 'HPF_fac', 'SST', 'MLD', 'SOI', 'PDO', 
			 "s(Lon)", "s(Moon, bs='cc')", "s(Yday, bs='cc')", "s(Hour, bs='cc')")
	delta_dev_expl_thresh <- 0.0025
	data_set <- SWOShala			#str(SWOShalb)			#str(SWOShala)
  }

# best_binomial_gam <- binomial_gam(data_set, var_name, out_directory, delta_dev_expl_thresh)
...
# ---- STEP 1: fit "null" models. Use glm and glmer
    ...
    null_aic <- last_aic <- as.numeric(summary(run_glmer)$AICtab["AIC"])
    null_LL <- last_LL <- as.numeric(summary(run_glmer)$AICtab["logLik"])			# attributes(thing)$df <- summary(run_glmer)$logLik
    null_nparms <- last_nparms <- attributes(summary(run_glmer)$logLik)$df + 1
    null_dev_expl <- last_dev_expl <- (intercept_only_dev-null_dev)/intercept_only_dev
  ...
  # write out info on the "null" models
   ...
   keep_going = TRUE
   current_formula <- year_only_text
 #  current_formula <- "z ~ Year_fac + s(Hour, bs='cc') + s(Yday, bs='cc') + s(Lat) + HPF_fac"


 #    null_gam <- run_gam <- gamm( z ~ Year_fac, random=list(Permit_fac = ~1), 
 #		data = data_set, family=binomial, na.action = na.omit)			# plot(null_gam$lme)
...
# --- STEP 2: try to add each variable, get AIC, compute likelihood ratio test, pick the one variable to add
   ...
   for (i in 1:nrow(list_vars)) {
	...
	formula_text <- paste0(current_formula," + ",add_var)
	formula_formula <- as.formula(formula_text)
	run_gam <- gamm4(formula_formula, random=~(1|Permit_fac), data = data_set, family= 'binomial', 
	...
	mod_nparms <- round(sum(run_gam$gam$edf),2)+ 2
...
#  --- STEP 3: look at results, choose which to leave out   # list_vars$LR_chi2_p[1] <- 0.1
 ...
 #	refit the GAM, return the model object and the data in a list 
	...
	best_gam <- gamm4(best_formula_formula, random=~(1|Permit_fac), data = data_set, family= 'binomial', 
		knots = list(Moon=c(0,1), Hour=c(0,24), Yday=c(0,366)), na.action = na.omit)
	return(best_gam)
...
# list vars for GAMs, specify which ones are circular or spline
#	var_name = c( "s(Lat)", 'Bait_fac', 'HPF_fac', 'SST', 'MLD', 'SOI', 'PDO', 
			 "s(Lon)", "s(Moon, bs='cc')", "s(Yday, bs='cc')", "s(Hour, bs='cc')")

# shallow_a_lnN_update <- lognormal_gam(data_set, var_name, out_directory, delta_dev_expl_thresh)
