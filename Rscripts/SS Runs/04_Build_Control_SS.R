...
Build_Control <- function(species,
  ...
  CTL        <- r4ss::SS_readctl_3.30(file = file.path(template_dir, "SWOcontrol.ctl"), 
                                      datlist = file.path(template_dir, "SWOdata.dat"))
  ...
  if(CTL$N_Block_Designs>=1) {
    ...
    c(ctl.sps[paste0("Block_Design_Start_",i)], ctl.sps[paste0("Block_Design_Finish_",i)])
  }
    }
  }                            
  ...
  if(CTL$natM_type==3){
    for (i in CTL$Nages){
      if(CTL$Nsexes==1){
        ...
        CTL$natM[i]==ctl.sps[paste0("Age_",i,"_F")]
 ...
 if(Nsexes != 2){
  ...
  r4ss::SS_writectl_3.30(CTL, outfile = file.path(current.dir, model.info$ctl.file.name), overwrite = TRUE)
